...
exports.handler = async function (event) {
  ...
  // Zelle confirmation attachments only
  const attachments = [];
  const zelleFiles = {
  ...
  const htmlCenter = `
    <div style="font-family:Arial,sans-serif;max-width:600px;color:#222;">
      <div style="background:#e8720c;padding:20px 24px;border-radius:8px 8px 0 0;">
        ...
        <p style="color:rgba(255,255,255,.85);margin:4px 0 0;font-size:12px;">A payment submission has been received.</p>
      </div>
      <div style="background:#fff;border:1px solid #f0d9c0;border-top:none;padding:24px;border-radius:0 0 8px 8px;">
        ...
        ${attachments.length ? `<p style="margin-top:10px;font-size:.85rem;color:#555;">📎 ${attachments.length} Zelle confirmation(s) attached.</p>` : ''}
      </div>
    </div>`;
